            ...
            if (existingSubscription) {
                if (!existingSubscription.subscribeStatus) {
                    ...
                    const subscribeNotification = new Notification({
                        ...
                        message: "Anda telah berhasil mengaktifkan kembali langganan layanan air.",
                        category: "INFORMASI",
                    });
...
export const incrementUsedWater = async (req, res) => {
    try {
        ...
        const isDue = checkBillingDue(subscription.updatedAt, waterCredit.billingTime);
        if (!isDue) {
            return res.status(200).json({
                status: 200,
        ...
        let tokenReward = 0;
        if (isTokenRewardDue && subscription.usedWaterInTempo < waterCredit.conservationToken.maxWaterUse) {
        ...
        try {
            ...
            if (wallet.balance < totalCost) {
                ...
                const remainingLiters = subscription.usedWaterInTempo - affordableLiters;
                ...
                const existingNotification = await checkNotificationExists(userId, "Peringatan Saldo", "TRANSAKSI");
                ...
                if (!existingNotification) {
                    ...
                    return res.status(200).json({
                        status: 200,
                        message: "Pembayaran sebagian berhasil, saldo tidak mencukupi untuk pembayaran penuh",
                        data: {
                            subscription,
                            wallet,
                            notification,
...
export const isBalanceZero = async (req, res) => {
    try {
        ...
        if (wallet.balance === 0) {
            ...
            const existingNotification = await checkNotificationExists(userId, "Pipa Ditutup", "INFORMASI");
...
export const getTransactionFromBlockchainByUserId = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            status: 200,
            message: "Successfully retrieved blockchain transactions",
